...
export default function ClubDetail() {
  ...
  return (
    <main className="min-h-screen text-black">
      <div className="mx-auto mt-12 flex w-full max-w-254 flex-col px-5 pb-20 lg:mt-25 xl:px-0">
        <Breadcrumb
          items={[
            { label: '홈', to: '/' },
            { label: clubDetail.university.name, to: `/universities/${clubDetail.university.id}/clubs` },
            { label: clubDetail.name },
          ]}
        />
        <div className="mt-10 grid gap-8 md:mt-15 md:grid-cols-[279px_minmax(0,1050px)] lg:gap-5">
              ...
              <div className="flex gap-10 py-[14.5px]">
                ...
                <div className="flex flex-col gap-1 leading-10">
                  ...
                  <div className="flex items-center gap-2 text-[16px]">
                    ...
                    <span className="text-text-600 min-w-0 truncate font-medium">{clubDetail.topic}</span>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-5 text-[18px] font-semibold">
                <h2 className="text-text-500">한줄소개</h2>
                <p className="text-text-900">{clubDetail.description}</p>
              </div>
            </section>
            <section className="border-text-100 flex flex-col gap-2.5 rounded-[20px] border bg-white px-11 py-10">
              <h2 className="text-text-900 text-[20px] font-bold">동아리 소개</h2>
