...
export default function UniversityDropdown({
  ...
  return (
    <div className="relative w-full">
      ...
      {isOpen && (
        <div className="border-text-100 absolute top-full left-0 z-10 mt-2 flex w-full flex-col items-start gap-5 self-stretch rounded-[20px] border bg-white px-7.5 py-6.5">
          {options.map((option) => (
            <button
              key={option.id}
              type="button"
